...
const queries = {
    ...
    crearPelicula:
        "INSERT INTO peliculas(titulo, id_imagen, anio, director, genero, duracion) VALUES($1, $2, $3, $4, $5, $6) RETURNING titulo, id_imagen, anio, director, genero, duracion",

    actualizarPeliculaById:
        "UPDATE peliculas SET titulo = $1, url_imagen = $2, anio = $3, director = $4, genero = $5, duracion = $6 WHERE id_pelicula = $7 RETURNING titulo, url_imagen, anio, director, genero, duracion",

    eliminarPeliculaTablaPeliculas:
        "DELETE FROM peliculas WHERE id_pelicula = $1",

    eliminarPeliculaTablaFavoritos:
        "DELETE FROM favoritos WHERE id_pelicula = $1",
    // FIN QUIERIES DE CRUD ADMIN

    findfavoritoByFavoritoId:
    ...
    findPeliculabyId:
        "SELECT id_pelicula as id,url_imagen as imagen,titulo anio,director,genero,duracion FROM peliculas WHERE id_pelicula=$1",

    findUserbyID: "SELECT * FROM usuarios WHERE id_usuario=$1",

    searchPelicula:
        "SELECT peliculas.url_imagen, peliculas.titulo,peliculas.anio,peliculas.director, peliculas.genero,peliculas.duracion FROM peliculas  WHERE peliculas.titulo ILIKE $1",

    detalleById:
        "SELECT peliculas.url_imagen, peliculas.titulo,peliculas.anio,peliculas.director, peliculas.genero,peliculas.duracion FROM peliculas WHERE peliculas.id_pelicula=$1",

    addFavorito:
        "INSERT INTO favoritos (id_usuario, id_pelicula) VALUES ($1, $2)",

    deleteFavorito:
        "DELETE FROM favoritos WHERE id_usuario=$1 AND id_favorito=$2",
